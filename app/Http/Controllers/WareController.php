...
class WareController extends Controller
{
	...
	public function index() {
		...
		if ($level > 1 ) {
			$warehouses = Ware::where('member_id', $member)->orderBy('nama', 'ASC')->get();
		} else {
			$warehouses = Ware::orderBy('member_id', 'ASC')->get();
		}

		$member = Member::where('id','<>','999')->where('id','<>','998')->where('id','<>','997')->get();

		return view('ware.tampil', compact('menus','warehouses','member','messages','messagecount','not','notcount')); 

	}

	public function wr2_index2(Request $request) {
		...
		if(request()->ajax()) {
			if(!empty($request->filter_gender)) {
				...
				if ($level > 1 ) {
					$warehouse = Ware::with('member')->where('member_id','=',$member)->orderBy('nama', 'ASC')->get();
				} else {
					$warehouse = Ware::with('member')->orderBy('nama', 'ASC')->get();
				}
		 	}

			return datatables()->of($warehouse)
					->addColumn('action', 'ware.action')
					->rawColumns(['action'])
					->addIndexColumn()
					->make(true);
		...
		return view('ware.tampil', compact('warehouse','members','accounts')); 
	
	}

	// handle insert ajax request
	public function wr2_store(Request $request) {
	...
	public function wr2_edit(Request $request) {		
		$id = $request->id;
		$emp = Ware::find($id);
	...
	public function wr2_update(Request $request) {
		
		$emp = Ware::find($request->emp_id);
		$empData = ['nama' => $request->nama, 
                    'status'=>$request->status,
		            'keterangan'=>$request->keterangan,
					'member_id'=>$request->member_id];
	...
	public function wr2_delete(Request $request) {
		$id = $request->id;
		$emp = Ware::find($id);
		if ($emp->status == "AKTIF" ) 
		{
	...
	//Export xls
	public function wr2_export() {

        $member = auth()->user()->member_id;
        $level = auth()->user()->level;
        $warehouses = Ware::orderBy('nama', 'ASC')->where('member_id','=',$member)->get();
		return view('ware.export',compact('warehouses'));
	}
	...
	public function wr2_pdf() {
		...
		$pdf = PDF::loadView('ware.lappdf', compact('warehouses','judul'))->setPaper('a4', 'potrait');
		return $pdf->stream();
	}
