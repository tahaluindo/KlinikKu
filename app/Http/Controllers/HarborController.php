...
class HarborController extends Controller
{
	...
	public function index() {
		...
		$harbors = Harbor::orderBy('nama', 'ASC')->get();
		$member = Member::where('id','<>','999')->where('id','<>','998')->where('id','<>','997')->get();

		return view('pelabuhan.tampil', compact('menus','harbors','member','messages','messagecount','not','notcount')); 

	}

	public function pl_index2(Request $request) {
		...
		if(request()->ajax()) {
			if(!empty($request->filter_gender)) {
				$harbor = Harbor::with('member')
							->where('status', $request->filter_gender)
							->orderBy('nama', 'ASC')
							->get();
		 	} else {
				$harbor = Harbor::with('member')->orderBy('nama', 'ASC')->get();
		 	}

			return datatables()->of($harbor)
		...
		$harbor = Harbor::with('member')->orderBy('nama', 'ASC')->get();
		return view('pelabuhan.tampil', compact('harbor','members','accounts')); 
	
	}

	// handle insert ajax request
	public function pl_store(Request $request) {
	...
	public function pl_edit(Request $request) {		
		$id = $request->id;
		$emp = Harbor::find($id);
	...
	public function pl_update(Request $request) {
		
		$emp = Harbor::find($request->emp_id);
		$empData = ['nama' => $request->nama, 
                    'status'=>$request->status,
		            'keterangan'=>$request->keterangan,
					'member_id'=>$request->member_id];
	...
	public function pl_export() {
        ...
        $member = auth()->user()->member_id;
        $level = auth()->user()->level;
        $boats = Harbor::orderBy('nama', 'ASC')->get();
		return view('pelabuhan.export',compact('boats'));
	}

	//Importxls
	public function pl_import(Request $request) {
    //    Excel::import(new SupplierImport, request()->file('filexlsimport'));
	//	return response()->json(['status' => 200]);
	}

	//Export pdf
	public function pl_pdf() {
		...
		$pdf = PDF::loadView('pelabuhan.lappdf', compact('boats','judul'))->setPaper('a4', 'potrait');
		return $pdf->stream();
	}
