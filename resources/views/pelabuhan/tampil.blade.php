    ...
    <table id="cashier_data" class="table table-bordered table-striped">
      <thead>
        ...
        <button type="button" name="filter" id="filter" class="btn btn-info" data-bs-toggle="tooltip" data-bs-placement="top" title="Filter Data"><i class="bi bi-filter"></i></button> 
            ...
            <a href="" onclick="this.href='pl_pdf'" button type="button" class="btn btn-danger" target="_blank" data-bs-toggle="tooltip" data-bs-placement="top" title="Export PDF"><i class="bi bi-file-pdf"></i></a>
            <a href="" onclick="this.href='pl_export'" button type="button" class="btn btn-success" target="_blank" data-bs-toggle="tooltip" data-bs-placement="top" title="Export Xls"><i class="bi bi-file-excel"></i></a>
        </th>
        </tr>
        <tr>
        <th scope="col" width="3%">ID</th>
        <th scope="col" width="30%">Pelabuhan</th>
          ...
          $.ajax({
            ...
            success: function(response) {
              if (response.status == 200) {
                Swal.fire(
                  'Added!',
                  'Pelabuhan Added Successfully!',
                  'success'
                )                
                var oTable = $('#cashier_data').dataTable();
                    oTable.fnDraw(false);
              }      
              $("#add_prod_btn").text('Add Pelabuhan');
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.status == 200) {
              Swal.fire(
                'Updated!',
                'Kapal Updated Successfully!',
                'success'
              )
              var oTable = $('#cashier_data').dataTable();
                  oTable.fnDraw(false);
            }
            $("#edit_prod_btn").text('Update Pelabuhan');
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.status == 200) {
              Swal.fire(
                'Updated!',
                'Kapal Imported Successfully!',
                'success'
              )
              var oTable = $('#cashier_data').dataTable();
                  oTable.fnDraw(false);
            }
            $("#import_prod_btn").text('Import Kapal');
