		...
		<div class="invoice-box">
			<table>
						...
						<table>
                            <tr>
                                <td colspan="2" align="center">
									...
									<h3>{{ $ju2 }} <br>LAPORAN KASIR - {{ $cashier->bank }} <br>PROYEK : {{ $ju }}</h3> 
				...
				<tr class="heading">
					...
					<td width="41%" align="left">Keterangan</td>
					
                    <td width="10%" class="heading2">Masuk</td>
					<td width="10%" class="heading2">Keluar</td>
					<td width="16%" class="heading2">Saldo</td>
					...
					@php $no = 1 @endphp
                        ...
                        <td width="12%" class="item"></td>
						<td width="10%" class="item"></td>
						<td width="41%" class="item">SALDO AWAL</td>
						
                        <td width="10%" class="item3">-</td>
                        <td width="10%" class="item3">-</td>
						<td width="16%" class="item3">{{ number_format($saldo)  }}</td>
						</tr>
...
$
						...
						<td width="41%" class="item">{{ $row->keterangan }}</td>
						
                        <td width="10%" class="item3">{{ number_format($row->masuk) }}</td>
                        <td width="10%" class="item3">{{ number_format($row->keluar) }}</td>
						<td width="16%" class="item3">{{ number_format(@array_sum($saldo_semua))  }}</td>
						</tr>
				...
				<tr class="item last">
                    <td colspan="4"><b>SALDO AKHIR</td>
					
					<td colspan="1"><b>{{ number_format(@array_sum($masuk_semua)) }}</td>
					<td colspan="1"><b>{{ number_format(@array_sum($keluar_semua)) }}</td>
					<td colspan="1"><b>{{ number_format(@array_sum($saldo_semua)) }}</td>
					<td></td>
				</tr>
