...
<html lang="en">
	<head>
		...
		<title>Laporan Cashflow Proyek</title>
		...
		<div class="invoice-box">
			<table>
						...
						<table>
                            <tr>
                                <td colspan="2" align="center">
									...
									<h3>PT. ANDIKA <br>LAPORAN CASHFLOW PROYEK<br>PROYEK : {{ $ju }}</h3> 
				...
				<tr class="heading">
                    ...
                    <td width="10%" align="left">No Trx</td>
					<td width="14%" align="left">Nama Bank</td>
					<td width="30%" align="left">Keterangan</td>
					
                    <td width="10%" class="heading2">Masuk</td>
					<td width="10%" class="heading2">Keluar</td>
					<td width="10%" class="heading2">Saldo</td>
					<td width="10%" class="heading2">Keterangan</td>
					...
					@php $no = 1 @endphp
						...
						<td width="30%" class="item">SALDO AWAL</td>
						
                        <td width="10%" class="item3">-</td>
                        <td width="10%" class="item3">-</td>
						<td width="10%" class="item3">{{ number_format($saldo)  }}</td>
						<td width="10%" class="item3">-</td>
						</tr>
...
$
						@php
							...
							$saldo_semua[]=$row->masuk-$row->keluar;
						@endphp
						<tr class="item">
                        <td width="1%"  class="item">{{ $no++ }} </td>
                        <td width="10%" class="item">{{ date_format($row->created_at,"d-M-Y") }}</td>
						<td width="10%" class="item">{{ $row->payinvoice->note }}</td>
						<td width="14%" class="item">{{ $row->cashier->bank }}</td>
						<td width="30%" class="item">{{ $row->keterangan }}</td>
						
                        <td width="10%" class="item3">{{ number_format($row->masuk) }}</td>
                        <td width="10%" class="item3">{{ number_format($row->keluar) }}</td>
						<td width="10%" class="item3">{{ number_format(@array_sum($saldo_semua))  }}</td>
						<td width="10%" class="item3">-</td>
						</tr>
				...
				<tr class="item last">
					...
					<td colspan="1">{{ number_format(@array_sum($masuk_semua)) }}</td>
